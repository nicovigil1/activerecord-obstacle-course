...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '3. finds order id of largest order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    order_id = Order.order(:amount).reverse_order.first.id
    # ------------------------------------------------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:items).where('item_id = ?', item_8.id).distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    data = User.joins(orders: :order_items)
            .select("users.name AS user_name", "orders.id AS order_id", "count(order_items.id) AS item_count").group(:order_id)
            .order(name: :desc)
    # ---------------------------------------------------------------
